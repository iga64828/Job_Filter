...
def get_openai_client(api_url: str = None):
    ...
    if not api_key:
        raise ValueError(
            "OPENAI_API_KEY environment variable is not set. Please set it to use the LLM matcher."
        )
...
def evaluate_job_with_resume(resume_text: str, job_info: dict) -> dict:
    ...
    system_msg = config_dt.get(
        "system_message",
        "You are a helpful AI assistant that outputs carefully structured JSON in Traditional Chinese format.",
    )
